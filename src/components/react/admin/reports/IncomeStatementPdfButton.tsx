...
export default function IncomeStatementPdfButton({
  initialMonth = new Date().getMonth() + 1,
  initialYear = new Date().getFullYear(),
}: IncomeStatementPdfButtonProps) {
  ...
  const pdfUrl = useMemo(() => {
    ...
    return `${API_URL}/report/income-statement/pdf?${params.toString()}`;
  }, [month, year]);
  ...
  return (
    <div className="w-full max-w-3xl rounded-2xl border border-primary-200 bg-white p-4 shadow-sm">
      <div className="grid grid-cols-1 md:grid-cols-[1fr_1fr_auto] gap-3 items-end">
        ...
        >
          <FaFilePdf size={14} /> Estado de resultado
        </Button>
      </div>
    </div>
  );
